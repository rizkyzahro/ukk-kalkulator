...
function evaluateExpression() {
  try {
    ...
    // *Gunakan titik sebagai desimal untuk perhitungan*
    expression = expression.replace(/,/g, ".");

    // Evaluasi ekspresi matematika
    const result = eval(expression);

    // *Konversi hasil ke format dengan koma sebelum ditampilkan*
    const formattedResult = result.toString().replace(/\./g, ",");

    addToHistory(screen.value, formattedResult); // Simpan ekspresi dan hasil dengan koma
...
  }
  ...
  const result = Math.pow(base, parseFloat(exponent)); // Hitung hasil
  const expression = `${base} ^ ${exponent}`; // Buat ekspresi asli
  addToHistory(expression, formatNumber(result)); // Tambahkan ke riwayat dengan hasil yang diformat
  screen.value = formatNumber(result); // Perbarui layar dengan hasil yang diformat
...
function log() {
  // Cek apakah screen.value kosong
  if (!screen.value.trim()) {
    alert("Input tidak boleh kosong!");
    return;
  }

  // Ubah koma menjadi titik agar parseFloat bekerja dengan benar
  const value = parseFloat(screen.value.replace(",", "."));
  
  // Cek apakah hasil parseFloat adalah angka positif
  if (isNaN(value) || value <= 0) {
    alert("Input harus angka positif!");
    return;
  }
  
  // Gunakan Math.log10() untuk logaritma basis 10 (bukan Math.log() yang pakai basis e)
